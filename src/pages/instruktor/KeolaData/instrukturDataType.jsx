...
const InstrukturDataType = () => {
  ...
  // Fetch type courses from Redux store
  ...
  // Pagination logic: slicing type courses for the current page
  const totalPages = Math.ceil(typeCourses?.length / itemsPerPage) || 1;
  const currentItems = typeCourses?.slice(
                ...
                {currentItems?.map((type, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* <td className="px-2 md:px-4 py-2">{type.categoryCode}</td> */}
                      <td className="px-2 md:px-4 py-2">{type.typeName}</td>
                    </tr>
                  );
                })}

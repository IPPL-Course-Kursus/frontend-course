...
const InstrukturDataInterpreter = () => {
  ...
  // Fetch interpreter languages from Redux store
  ...
  // Pagination logic: slicing languages for the current page
  const totalPages = Math.ceil(languages?.length / itemsPerPage) || 1;
  const currentItems = languages?.slice(
            ...
            <table className="min-w-full table-auto">
              ...
              <tbody>
                {currentItems?.map((language, index) => {
                  ...
                  return (
                    <tr
                      key={language.id} // Use unique identifier for key
                    ...
                    >
                      ...
                      {/* <td className="px-2 md:px-4 py-2">{language.categoryCode}</td> */}
                      <td className="px-2 md:px-4 py-2">
                        {language.languageInterpreter}
                      </td>
                ...
                {currentItems && currentItems.length === 0 && (
                  <tr>
                    <td colSpan="3" className="text-center py-4 text-gray-500">
                      Tidak ada bahasa interpreter yang ditemukan.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="block md:hidden space-y-6">
            {currentItems?.length > 0 ? (
              currentItems.map((language, index) => (
                ...
                >
                  {/* Header: Nomor urutan dan nama bahasa interpreter */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-lg font-bold text-gray-700">
                      {((currentPage - 1) * itemsPerPage + index + 1).toString()}. {language.languageInterpreter}
                    </div>
                  </div>
                  ...
                  {/* Versi */}
                  <div className="mb-3 text-sm text-gray-600">
                    <span className="font-semibold text-gray-800">Versi:</span>{" "}
                    {language.version}
                  </div>
                </div>
              ))
...
// Define PropTypes for InstrukturDataInterpreter if needed
InstrukturDataInterpreter.propTypes = {

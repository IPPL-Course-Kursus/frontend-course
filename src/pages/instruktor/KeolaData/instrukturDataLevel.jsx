...
const InstrukturDataLevel = () => {
  ...
  // Fetch level courses from Redux store
  const { levelCourses, loading, error, successMessage } = useSelector(
    (state) => state.levelCourse
  );
  ...
  // Pagination logic: slicing level courses for the current page
  const totalPages = Math.ceil(levelCourses?.length / itemsPerPage) || 1;
  const currentItems = levelCourses?.slice(
            ...
            <table className="min-w-full table-auto">
              ...
              <tbody>
                {currentItems?.map((level, index) => {
                  ...
                  return (
                    <tr
                      key={level.id} // Use unique identifier for key
          ...
          <div className="block md:hidden space-y-6">
            {currentItems?.length > 0 ? (
              currentItems.map((level, index) => (
                ...
                >
                  {/* Header: Nomor urutan dan nama level */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-lg font-bold text-gray-700">
                      {((currentPage - 1) * itemsPerPage + index + 1).toString()}. {level.levelName}
                    </div>
              ...
              <p className="text-center text-gray-500">Tidak ada level yang ditemukan.</p>
            )}
          </div>
        </div>
...
// Define PropTypes for InstrukturDataLevel if needed
InstrukturDataLevel.propTypes = {

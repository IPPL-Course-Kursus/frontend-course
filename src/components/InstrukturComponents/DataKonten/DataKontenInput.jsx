...
const DataKontenModule = ({ show, onClose, chapterId }) => {
  ...
  const handleAdd = (e) => {
    ...
    const requestData = {
      ...
      contentUrl,
      duration: Number(duration),
      interpreterId: interpreterId || null, // Use null if interpreterId is not provided
      interpreterStatus, // Include interpreterStatus
    };
    ...
    dispatch(addDataKonten(requestData, chapterId))
      .then(() => {
        setLoading(false);
        onClose();
        window.location.reload(); // Reload window after successful addition
      })
      .catch((err) => {
        setLoading(false);
        setError(err.response?.data?.message || "Error adding content");
